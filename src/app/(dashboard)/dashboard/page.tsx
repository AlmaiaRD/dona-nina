...
const PIE_COLORS = ["#5B9E6B", "#7C1D2E", "#D4A017"];
        ...
        <>
          ...
          <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8] mb-8">
            <div className="flex items-center justify-between mb-4">
              <h3 className="text-sm font-semibold text-[#3D2B1F]">Meta Mensual</h3>
            ...
            <div className="bg-[#FCFAF7] border border-dashed border-[#E8E0D8] rounded-xl p-3 mb-4 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
              ...
              <div className="flex items-center gap-2">
                <input
                  ...
                  placeholder="0"
                  className="text-lg font-extrabold text-[#3D2B1F] border-none bg-transparent w-36 text-right outline-none placeholder:text-[#E8E0D8]"
                />
                <button
                  type="button"
                  onClick={() => {
                  ...
                  className="text-xs bg-[#7C1D2E] text-white px-4 py-2 rounded-lg font-semibold hover:bg-[#5C1420] active:scale-95 transition-all cursor-pointer"
                >
                  Fijar Meta
                </button>
              </div>
            ...
            <div className="flex mb-1.5 text-xs font-semibold text-[#3D2B1F]">
            ...
            <div className="flex h-9 rounded-xl overflow-hidden" style={{ boxShadow: "inset 0 1px 3px rgba(0,0,0,0.06)" }}>
              <div
                ...
                style={{ width: `${barCobrado}%`, backgroundColor: "#5B9E6B" }}
              >
                {cobrado > 0 ? formatCurrency(cobrado) : ""}
              </div>
              <div
                className="flex items-center justify-center text-xs font-bold text-white transition-all duration-500"
                style={{ width: `${barVendido}%`, backgroundColor: "#7C1D2E" }}
              >
            ...
            <div className="flex flex-col sm:flex-row gap-3 mt-4">
              <div className="flex-1 bg-[#FCFAF7] rounded-xl p-3 text-center border border-[#E8E0D8]">
                ...
                <div className="text-base font-extrabold text-[#5B9E6B] mt-1">{formatCurrency(cobrado)}</div>
              ...
              <div className="flex-1 bg-[#FCFAF7] rounded-xl p-3 text-center border border-[#E8E0D8]">
                ...
                <div className="text-base font-extrabold text-[#7C1D2E] mt-1">{formatCurrency(vendido)}</div>
              ...
              <div className="flex-1 bg-[#FCFAF7] rounded-xl p-3 text-center border border-[#E8E0D8]">
                ...
                <div className="text-base font-extrabold text-[#3D2B1F] mt-1">{formatCurrency(restanteKpi)}</div>
          ...
          <div className="mb-8">
            <h3 className="text-sm font-semibold text-[#3D2B1F] mb-4">Módulos</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
              {[
                ...
                <button
                  ...
                  className="flex flex-col items-center gap-3 p-5 bg-white rounded-2xl shadow-sm border border-[#E8E0D8] hover:shadow-md hover:border-[#7C1D2E]/30 transition-all"
                >
                  <div className={`p-3 rounded-xl ${mod.color}`}>
                    <mod.icon size={22} />
                  </div>
                  <span className="text-sm font-medium text-[#3D2B1F]">{mod.label}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-6">
            <div className="lg:col-span-2 bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8]">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-[#3D2B1F]">Comparativa Mensual</h3>
                ...
                {monthData.length === 0 ? (
                  ...
                  <ResponsiveContainer width="100%" height="100%">
                    <BarChart data={monthData} barGap={4}>
                      ...
                      />
                      <Bar dataKey="ventas" fill="#7C1D2E" radius={[6, 6, 0, 0]} name="Ventas" />
                      <Bar dataKey="cobros" fill="#5B9E6B" radius={[6, 6, 0, 0]} name="Cobros" />
                    </BarChart>
                  </ResponsiveContainer>
                )}
              </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8]">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-[#3D2B1F]">Métodos de Pago</h3>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8]">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-[#3D2B1F]">Ventas Diarias</h3>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <LineChart data={dailySales}>
                    ...
                    <Tooltip
                      ...
                      formatter={(value: any) => formatCurrency(Number(value))}
                    />
                    <Line type="monotone" dataKey="ventas" stroke="#7C1D2E" strokeWidth={2} dot={{ fill: "#7C1D2E", r: 3 }} />
                  </LineChart>
                </ResponsiveContainer>
              </div>
            </div>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8]">
              <div className="flex items-center gap-2 mb-4">
                ...
                <h3 className="text-sm font-semibold text-[#3D2B1F]">Alertas de Stock</h3>
              </div>
              {lowStock.length === 0 ? (
                ...
                <div className="space-y-3">
                  {lowStock.map((p) => (
                    <div key={p.name} className="flex items-center justify-between p-3 bg-[#FFFBEB] rounded-xl border border-[#D4A017]/30">
                      <div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8]">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-[#3D2B1F]">Facturas Recientes</h3>
                <button onClick={() => router.push("/facturacion")} className="text-xs text-[#7C1D2E] hover:underline flex items-center gap-1">
                  ...
                  {recentInvoices.map((inv: any) => (
                    <div key={inv.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-medium text-[#3D2B1F]">{formatCurrency(Number(inv.total))}</p>
                        <span className={`text-xs font-medium ${
                          inv.status === "PAID" ? "text-[#5B9E6B]" : inv.status === "PENDING" ? "text-[#D4A017]" : "text-[#9C8A82]"
                        }`}>
            ...
            <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8]">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-[#3D2B1F]">Recibos Recientes</h3>
                <button onClick={() => router.push("/recibos")} className="text-xs text-[#5B9E6B] hover:underline flex items-center gap-1">
                ...
                <div className="divide-y divide-[#F0EBE3]">
                  {recentReceipts.map((rec: any) => (
                    <div key={rec.id} className="flex items-center justify-between py-3 first:pt-0 last:pb-0">
                      ...
                      <div className="text-right">
                        <p className="text-sm font-medium text-[#5B9E6B]">{formatCurrency(Number(rec.amount))}</p>
