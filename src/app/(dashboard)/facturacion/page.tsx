    ...
    <PageContainer
      ...
      action={
        <button
          ...
          className="flex items-center gap-2 bg-red-600 text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-red-700 transition-all duration-200 shadow-sm"
        >
          <Plus size={18} />
          Nueva Factura
        </button>
        ...
        {selectedInvoice && (
          <div className="space-y-5">
            <div id="invoice-preview" className="bg-white p-8 rounded-xl" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
              ...
              <div className="flex justify-between items-start mb-6">
                <div className="flex items-start gap-2">
                  <div className="w-14 h-14 rounded-full bg-red-600/10 flex items-center justify-center mt-1 text-3xl">
                    🍰
                  </div>
                  <div>
                    <h2 className="text-xl font-extrabold text-gray-900" style={{ letterSpacing: '-0.02em' }}>Donde Doña Nina</h2>
                    <p className="text-xs tracking-widest text-red-600 uppercase mt-0.5 font-semibold">Hechas con el amor de mamá</p>
                    <p className="text-[10px] text-gray-500 mt-0.5 font-medium">Sabor Dominicano</p>
                  </div>
                </div>
                <div className="text-right">
                  <span className="inline-block bg-gray-50 text-red-600 text-xs font-bold px-4 py-2 rounded-full">FACTURA DE VENTA</span>
                  <p className="text-lg font-bold text-gray-900 mt-3">{selectedInvoice.invoice_number}</p>
                  <p className="text-xs text-gray-500 mt-0.5">Fecha: {formatDate(selectedInvoice.invoice_date)}</p>
                  <div className="mt-2">
              ...
              <div className="border border-gray-200 bg-gray-50 rounded-xl p-4 mb-5">
                <p className="text-xs font-bold text-red-600 mb-3">CLIENTE / ADQUIRIENTE</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm">
                  <p className="text-gray-900"><span className="text-gray-500">Nombre:</span> {selectedInvoice.client?.full_name}</p>
                  <p className="text-gray-900"><span className="text-gray-500">Tel\u00e9fono:</span> {selectedInvoice.client?.phone || "\u2014"}</p>
                  <p className="text-gray-900"><span className="text-gray-500">Email:</span> {selectedInvoice.client?.email || "N/D"}</p>
                  {selectedInvoice.client?.address && (
                    <p className="text-gray-900"><span className="text-gray-500">Direcci\u00f3n:</span> {selectedInvoice.client.address}</p>
                  )}
                </div>
              </div>
              ...
              <table className="w-full text-sm mb-5">
                <thead>
                  <tr className="bg-gray-50">
                    <th className="py-2.5 px-3 text-left text-xs text-gray-900 font-bold">Categor\u00eda</th>
                    <th className="py-2.5 px-3 text-left text-xs text-gray-900 font-bold">Descripci\u00f3n / Producto</th>
                  ...
                  {(selectedInvoice.items || []).map((item: any, i: number) => {
                    return (
                      <tr key={i} className="border-b border-gray-100">
                        <td className="py-2.5 px-3 text-xs text-gray-500">{item.menu_item?.category?.name || "\u2014"}</td>
              ...
              {selectedInvoice.bank_accounts && (
                <div className="border border-gray-200 bg-gray-50 rounded-xl p-4 mb-5">
                  <p className="text-xs font-bold text-red-600 mb-3">DATOS DE TRANSFERENCIA</p>
                  <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm">
                    <p className="text-gray-900"><span className="text-gray-500">Beneficiario:</span> {selectedInvoice.bank_accounts.holder_name}</p>
                    {selectedInvoice.bank_accounts.id_number && <p className="text-gray-900"><span className="text-gray-500">C\u00e9dula/RNC:</span> {selectedInvoice.bank_accounts.id_number}</p>}
                    <p className="text-gray-900"><span className="text-gray-500">Banco:</span> {selectedInvoice.bank_accounts.bank_name}</p>
                    <p className="text-gray-900"><span className="text-gray-500">Tipo de Cuenta:</span> {selectedInvoice.bank_accounts.account_type}</p>
                    <p className="text-gray-900"><span className="text-gray-500">No. de Cuenta:</span> {selectedInvoice.bank_accounts.account_number}</p>
                    {selectedInvoice.bank_accounts.email && <p className="text-gray-900"><span className="text-gray-500">Correo:</span> {selectedInvoice.bank_accounts.email}</p>}
                  </div>
                </div>
              )}
              ...
              <div className="border-t border-gray-200 pt-3 mb-5">
                ...
                <div className="flex justify-between text-base font-bold text-gray-900 pt-1 border-t border-gray-200 mb-1">
                  <span>Total General</span>
              ...
              {selectedInvoice.notes && (
                <div className="bg-amber-50 border border-gray-200 rounded-xl p-3 mb-5">
              ...
              <div className="border-t border-gray-200 pt-4 flex justify-between items-end">
                <div>
                  <p className="text-xs italic text-red-600 whitespace-nowrap">¡Gracias por tu compra! Hecho con el amor de mamá.</p>
                  <p className="text-[10px] text-gray-500 mt-1.5 whitespace-nowrap">Donde Doña Nina · Hechas con el amor de mamá · Sabor Dominicano</p>
                </div>
                <div className="text-right">
                  {settings?.signature_url ? (
            ...
            <div className="flex flex-wrap gap-3">
              <button onClick={() => handlePrintPdf(selectedInvoice)} className="flex-1 min-w-[120px] h-12 border border-gray-200 text-gray-900 rounded-xl text-sm font-medium hover:bg-gray-50 transition-all flex items-center justify-center gap-2">
                <FileText size={18} /> PDF
              </button>
              <button onClick={() => handlePrintJpg(selectedInvoice)} className="flex-1 min-w-[120px] h-12 border border-gray-200 text-gray-900 rounded-xl text-sm font-medium hover:bg-gray-50 transition-all flex items-center justify-center gap-2">
                <Download size={18} /> JPG
              </button>
              {selectedInvoice.client?.email && (
                <button
                  onClick={() => setDraftModal({ type: "email" })}
                  className="flex-1 min-w-[120px] h-12 border border-gray-200 text-gray-900 rounded-xl text-sm font-medium hover:bg-gray-50 transition-all flex items-center justify-center gap-2"
                >
                  <Mail size={18} /> Email
                </button>
              )}
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div className="col-span-2">
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Nombre completo *</label>
              <input type="text" value={newClientForm.full_name} onChange={(e) => setNewClientForm({ ...newClientForm, full_name: e.target.value })} placeholder="Nombre y apellidos" className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Teléfono</label>
              <input type="text" value={newClientForm.phone} onChange={(e) => setNewClientForm({ ...newClientForm, phone: e.target.value })} placeholder="809-000-0000" className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Correo electrónico</label>
              <input type="email" value={newClientForm.email} onChange={(e) => setNewClientForm({ ...newClientForm, email: e.target.value })} placeholder="correo@ejemplo.com" className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Dirección</label>
              <input type="text" value={newClientForm.address} onChange={(e) => setNewClientForm({ ...newClientForm, address: e.target.value })} placeholder="Dirección" className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Notas</label>
              <input type="text" value={newClientForm.notes} onChange={(e) => setNewClientForm({ ...newClientForm, notes: e.target.value })} placeholder="Notas" className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Cumpleaños</label>
              <input type="date" value={newClientForm.birthday} onChange={(e) => setNewClientForm({ ...newClientForm, birthday: e.target.value })} className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Etapa</label>
              <select value={newClientForm.stage} onChange={(e) => setNewClientForm({ ...newClientForm, stage: e.target.value })} className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all">
            ...
            <div>
              ...
              <input type="text" value={newClientForm.lead_source} onChange={(e) => setNewClientForm({ ...newClientForm, lead_source: e.target.value })} placeholder="Ej: Referido, Redes, Whatsapp" className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Interés</label>
              <input type="text" value={newClientForm.interest} onChange={(e) => setNewClientForm({ ...newClientForm, interest: e.target.value })} placeholder="Ej: Pasteles, Postres, Salados" className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 placeholder-gray-500 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Fecha 1er Contacto</label>
              <input type="date" value={newClientForm.first_contact_date} onChange={(e) => setNewClientForm({ ...newClientForm, first_contact_date: e.target.value })} className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-900 mb-1.5">Próximo Seguimiento</label>
              <input type="date" value={newClientForm.next_followup_date} onChange={(e) => setNewClientForm({ ...newClientForm, next_followup_date: e.target.value })} className="w-full h-12 px-4 rounded-xl border border-gray-200 bg-gray-50 text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-red-500/30 focus:border-red-200 transition-all" />
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => { setShowNewClient(false); setNewClientForm({ full_name: "", phone: "", email: "", address: "", notes: "", birthday: "", stage: "Prospecto", lead_source: "", interest: "", first_contact_date: getLocalDateString(), next_followup_date: "" }); }} className="flex-1 h-12 border border-gray-200 text-gray-900 rounded-xl text-sm font-medium hover:bg-gray-50 transition-all">Cancelar</button>
            <button onClick={handleSaveNewClient} disabled={saving} className="flex-1 h-12 bg-red-600 text-white rounded-xl text-sm font-medium hover:bg-red-700 transition-all shadow-sm disabled:opacity-50 flex items-center justify-center gap-2">
        ...
        {jpgData && (
          <div id="invoice-preview" className="bg-white p-8" style={{ fontFamily: "Inter, system-ui, sans-serif" }}>
            <div className="flex justify-between items-start mb-6">
              ...
              <div className="text-right">
                <span className="inline-block bg-gray-50 text-red-600 text-xs font-bold px-4 py-2 rounded-full">FACTURA DE VENTA</span>
                <p className="text-lg font-bold text-gray-900 mt-3">{jpgData.invoice_number}</p>
                <p className="text-xs text-gray-500 mt-0.5">Fecha: {formatDate(jpgData.invoice_date)}</p>
              </div>
            </div>
            <div className="border-t border-gray-200 mb-5" />
            <div className="border border-gray-200 bg-gray-50 rounded-xl p-4 mb-5">
              <p className="text-xs font-bold text-red-600 mb-3">CLIENTE / ADQUIRIENTE</p>
              <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm">
                <p className="text-gray-900"><span className="text-gray-500">Nombre:</span> {jpgData.client?.full_name}</p>
                <p className="text-gray-900"><span className="text-gray-500">Tel\u00e9fono:</span> {jpgData.client?.phone || "\u2014"}</p>
                <p className="text-gray-900"><span className="text-gray-500">Email:</span> {jpgData.client?.email || "N/D"}</p>
              </div>
            </div>
            <table className="w-full text-sm mb-5">
              <thead>
            ...
            {jpgData.bank_accounts && (
              <div className="border border-gray-200 bg-gray-50 rounded-xl p-4 mb-5">
                <p className="text-xs font-bold text-red-600 mb-3">DATOS DE TRANSFERENCIA</p>
                <div className="grid grid-cols-2 gap-x-4 gap-y-1.5 text-sm">
                  <p className="text-gray-900"><span className="text-gray-500">Beneficiario:</span> {jpgData.bank_accounts.holder_name}</p>
                  {jpgData.bank_accounts.id_number && <p className="text-gray-900"><span className="text-gray-500">C\u00e9dula/RNC:</span> {jpgData.bank_accounts.id_number}</p>}
                  <p className="text-gray-900"><span className="text-gray-500">Banco:</span> {jpgData.bank_accounts.bank_name}</p>
                  <p className="text-gray-900"><span className="text-gray-500">Tipo de Cuenta:</span> {jpgData.bank_accounts.account_type}</p>
                  <p className="text-gray-900"><span className="text-gray-500">No. de Cuenta:</span> {jpgData.bank_accounts.account_number}</p>
                  {jpgData.bank_accounts.email && <p className="text-gray-900"><span className="text-gray-500">Correo:</span> {jpgData.bank_accounts.email}</p>}
                </div>
              </div>
            )}
            <div className="border-t border-gray-200 pt-3 mb-5">
