  ...
  return (
    <PageContainer>
      <div className="mb-6">
        ...
        <h1 className="text-xl font-bold text-[#3D2B1F]">Saldos a Favor</h1>
      ...
      <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8] mb-6">
        ...
        <p className="text-2xl font-bold text-[#5B9E6B]">{formatCurrency(totalAvailable)}</p>
      </div>
      ...
      {loading ? (
        <div className="flex justify-center py-16">
          <div className="w-8 h-8 border-2 border-[#5B9E6B] border-t-transparent rounded-full animate-spin" />
        </div>
      ) : filtered.length === 0 ? (
          ...
          {filtered.map((c) => (
            <div key={c.id} className="bg-white rounded-2xl p-4 shadow-sm border border-[#E8E0D8] hover:shadow-md transition-all">
              <div className="flex items-center justify-between mb-2">
                <div>
                  <p className="text-sm font-medium text-[#3D2B1F]">{c.clients?.full_name || "Sin cliente"}</p>
              ...
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <span className="text-sm text-[#3D2B1F] ml-1">{formatCurrency(Number(c.amount))}</span>
                </div>
                <button onClick={() => { setSelectedId(c.id); setApplyAmount(Number(c.amount)); }}
                  className="flex items-center gap-1 text-xs text-[#5B9E6B] hover:underline">
              ...
              {selectedId === c.id && (
                <div className="mt-3 pt-3 border-t border-[#F0EBE3] flex items-center gap-3">
                  <input type="number" value={applyAmount} max={Number(c.amount)}
                    ...
                    className="flex-1 h-10 px-3 rounded-xl border border-[#E8E0D8] text-sm text-[#3D2B1F] focus:outline-none focus:ring-2 focus:ring-[#5B9E6B]/30 focus:border-[#5B9E6B]" />
                  <button onClick={() => handleApply(c)} disabled={saving}
                    className="h-10 px-4 bg-[#5B9E6B] text-white rounded-xl text-sm font-medium hover:bg-[#6DB08A] transition-all shadow-sm disabled:opacity-50">
                    {saving ? "Aplicando..." : "Aplicar a Factura"}
                  </button>
                  <button onClick={() => setSelectedId(null)} className="text-xs text-[#9C8A82] hover:text-[#3D2B1F]">Cancelar</button>
                </div>
              )}
            </div>
          ))}
