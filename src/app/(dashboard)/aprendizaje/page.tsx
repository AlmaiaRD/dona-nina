    ...
    >
      <div className="space-y-5">
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-9 pr-4 py-2.5 text-sm border border-[#E8E0D8] rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#800020]/20 focus:border-[#800020] transition-colors"
            />
          </div>
        </div>
        ...
        {allTags.length > 0 && (
          <div className="flex flex-wrap items-center gap-2">
            <Tag className="h-4 w-4 text-[#9C8A82]" />
            {allTags.map((tag) => (
              <button
                key={tag}
            ...
            {filterTag && (
              <button
                ...
                className="text-xs text-[#9C8A82] hover:text-gray-600 underline"
              >
                Limpiar filtro
              </button>
            )}
        ...
        {filtered.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((note) => (
              <div
                key={note.id}
                className="bg-white rounded-xl shadow-sm border border-[#E8E0D8] p-5 flex flex-col"
              >
                {note.tags.trim() && (
                ...
                <h3 className="font-semibold text-[#3D2B1F] mb-1.5">{note.title}</h3>
                <p className="text-sm text-gray-600 leading-relaxed line-clamp-3 flex-1">
                  {note.content}
                </p>
                <div className="flex items-center justify-between mt-4 pt-3 border-t border-gray-50">
                  <span className="text-[11px] text-[#9C8A82]">{formatDate(note.updated_at)}</span>
                  <div className="flex items-center gap-1">
                    <button
                      onClick={() => openEdit(note)}
                      className="p-1.5 rounded-lg text-[#9C8A82] hover:text-[#d97706] hover:bg-[#FCFAF7] transition-colors"
                      title="Editar"
                    >
                    ...
                    <button
                      ...
                      className="p-1.5 rounded-lg text-[#9C8A82] hover:text-[#7C1D2E] hover:bg-[#7C1D2E]/10 transition-colors"
                      title="Eliminar"
                    >
      ...
      >
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1">Título</label>
            <input
              type="text"
              value={form.title}
              onChange={(e) => setForm({ ...form, title: e.target.value })}
              placeholder="Ej: Error con tipos en TypeScript"
              className="w-full px-3 py-2.5 text-sm border border-[#E8E0D8] rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#800020]/20 focus:border-[#800020] transition-colors"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1">Contenido</label>
            <textarea
              rows={6}
              value={form.content}
              onChange={(e) => setForm({ ...form, content: e.target.value })}
              placeholder="Describe lo que aprendiste..."
              className="w-full px-3 py-2.5 text-sm border border-[#E8E0D8] rounded-lg bg-white focus:outline-none focus:ring-2 focus:ring-[#800020]/20 focus:border-[#800020] transition-colors resize-y"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1">
              Etiquetas <span className="text-[#9C8A82] font-normal">(opcional)</span>
            </label>
            <input
              type="text"
              value={form.tags}
          ...
          <div className="flex items-center justify-end gap-3 pt-2">
            <button
              ...
              disabled={saving}
              className="px-4 py-2.5 text-sm font-medium text-[#3D2B1F] bg-[#FDF8F3] rounded-lg hover:bg-gray-200 transition-colors disabled:opacity-50"
            >
              Cancelar
            </button>
            <button
