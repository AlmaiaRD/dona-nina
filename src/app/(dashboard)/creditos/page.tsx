  ...
  return (
    <PageContainer
      ...
      action={
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2 text-sm font-medium text-[#3D2B1F] bg-white border border-[#E8E0D8] rounded-lg hover:bg-[#FDF8F3] transition-colors"
        >
    ...
    >
      ...
      <div className="bg-white rounded-xl p-5 shadow-sm border border-[#E8E0D8]">
        <p className="text-sm text-[#9C8A82] font-medium">Total Disponible</p>
              ...
              <div className="flex items-start justify-between gap-4">
                <div className="min-w-0 flex-1">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="font-semibold text-[#3D2B1F] truncate">
                ...
                <div className="mt-4 pt-4 border-t border-[#E8E0D8]">
                  <div className="flex items-end gap-3">
                    ...
                    <button
                      ...
                      disabled={saving}
                      className="inline-flex items-center gap-1.5 px-4 py-2 text-sm font-medium text-white bg-[#7C1D2E] rounded-lg hover:bg-[#5C1420] disabled:opacity-50 transition-colors"
                    >
                ...
                <div className="mt-3 pt-3 border-t border-gray-50">
                  <div className="flex items-center gap-3">
                    <button
                      ...
                      className="inline-flex items-center gap-1.5 text-sm font-medium text-gray-600 hover:text-[#3D2B1F] transition-colors"
                    >
                    ...
                    {credit.status === 'AVAILABLE' && (
                      <button
                        ...
                        className="inline-flex items-center gap-1.5 text-sm font-medium text-[#7C1D2E] hover:text-[#7C1D2E] transition-colors"
                      >
                        Aplicar
                        <ArrowRight className="h-4 w-4" />
                      </button>
        ...
        <div id="credit-detail" className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <p className="text-sm font-semibold text-[#3D2B1F]">{detailCredit?.client?.full_name || '-'}</p>
            </div>
            <div>
              <p className="text-xs text-[#9C8A82] font-medium">Monto Original</p>
              <p className="text-sm font-semibold text-[#3D2B1F]">{detailCredit ? formatCurrency(detailCredit.amount) : '-'}</p>
            </div>
            <div>
              <p className="text-xs text-[#9C8A82] font-medium">Balance Disponible</p>
              <p className="text-sm font-semibold text-[#3D2B1F]">{detailCredit ? formatCurrency(detailCredit.balance) : '-'}</p>
            </div>
            <div>
              <p className="text-xs text-[#9C8A82] font-medium">Recibo</p>
              <p className="text-sm font-semibold text-[#3D2B1F]">{detailCredit?.receipt_id || '-'}</p>
            </div>
            <div>
              <p className="text-xs text-[#9C8A82] font-medium">Fecha de Creación</p>
              <p className="text-sm font-semibold text-[#3D2B1F]">{detailCredit ? formatDate(detailCredit.created_at) : '-'}</p>
            </div>
            <div>
