...
const CLIENT_TYPE_CONFIG = {
  comprador: { label: "Clientes Compradores", icon: CartIcon, color: "text-[#5B9E6B]", bg: "bg-[#5B9E6B]/10", stages: SALES_STAGES },
...
export default function PipelinePage() {
  ...
  function KanbanCard({ client }: { client: ClientCardData }) {
    ...
    return (
      <div
        className={`bg-white rounded-xl border p-3 cursor-pointer hover:shadow-md transition-all ${isStagnant ? "border-[#D4A017]" : "border-[#E8E0D8]"} ${draggingId === client.id ? "opacity-50" : ""}`}
      ...
      >
        <div className="flex items-start justify-between gap-2 mb-2">
          <div className="flex items-center gap-2 min-w-0">
            {batchMode && <input type="checkbox" checked={selectedIds.has(client.id)} onChange={() => toggleSelect(client.id)} onClick={e => e.stopPropagation()} className="rounded border-[#E8E0D8] text-[#7C1D2E]" />}
            <div className="w-8 h-8 rounded-full bg-[#7C1D2E]/10 flex items-center justify-center flex-shrink-0">
              <User size={14} className="text-[#7C1D2E]" />
            </div>
            <div className="min-w-0">
              <h4 className="text-sm font-semibold text-[#3D2B1F] truncate">{client.full_name}</h4>
              {client.phone && (
                <a href={`https://wa.me/1${client.phone.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" onClick={e => e.stopPropagation()} className="text-[10px] text-[#9C8A82] hover:text-[#5B9E6B] flex items-center gap-1">
          ...
          <div className="flex items-center gap-1">
            {isStagnant && <span className="text-[9px] px-1.5 py-0.5 rounded bg-[#D4A017]/20 text-[#B8860B] font-medium whitespace-nowrap">⚠ {client.days_in_stage}d</span>}
            <button
              onClick={e => { e.stopPropagation(); setEditingNote(!editingNote); }}
              className="p-1 rounded hover:bg-[#FDF8F3] text-[#9C8A82] hover:text-[#7C1D2E] transition-colors"
        ...
        {editingNote ? (
          <div className="mt-2" onClick={e => e.stopPropagation()}>
            <textarea
              ...
              rows={3}
              className="w-full px-2 py-1.5 text-xs text-[#3D2B1F] bg-[#FCFAF7] border border-[#7C1D2E]/30 rounded-lg focus:outline-none focus:ring-1 focus:ring-[#7C1D2E] resize-none"
            />
            <p className="text-[9px] text-[#9C8A82] mt-1">Enter para guardar · Esc para cancelar</p>
          </div>
        ) : (
          client.notes && (
            <div className="mt-2 p-2 bg-[#FDF8F3] rounded-lg">
              <p className="text-[10px] text-[#3D2B1F] line-clamp-2 leading-relaxed">{client.notes}</p>
            </div>
          )
        )}
        ...
        <div className="flex items-center gap-2 text-[10px] text-[#9C8A82] mt-2">
          ...
          {client.pending_balance > 0 && (
            <span className="text-[#E07A3A] font-medium">Pend: {formatCurrency(client.pending_balance)}</span>
          )}
        </div>
        ...
        {client.tags.length > 0 && (
          <div className="flex flex-wrap gap-1 mt-2">
            {client.tags.slice(0, 2).map(t => (
              <span key={t.id} className="text-[9px] px-1.5 py-0.5 rounded-full bg-[#7C1D2E]/10 text-[#7C1D2E]">{t.name}</span>
            ))}
          </div>
        )}
      </div>
  ...
  return (
    <PageContainer>
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-xl font-bold text-[#3D2B1F]">Pipeline Comercial</h1>
          <p className="text-sm text-[#9C8A82] mt-1">Arrastra clientes entre etapas</p>
        </div>
        <div className="flex flex-wrap items-center gap-2">
          <button onClick={() => setShowAddModal(true)} className="flex items-center gap-2 bg-[#7C1D2E] text-white px-4 py-2.5 rounded-xl text-sm font-medium hover:bg-[#5C1420] transition-all shadow-sm">
            <UserPlus size={16} /> <span className="hidden sm:inline">Agregar al Pipeline</span><span className="sm:hidden">Agregar</span>
          </button>
          {batchMode && selectedIds.size > 0 && (
            <select onChange={e => { if (e.target.value) handleBatchMove(e.target.value); e.target.value = ""; }} className="h-10 px-3 rounded-lg border border-[#E8E0D8] bg-white text-xs text-[#3D2B1F]" defaultValue="">
              <option value="" disabled>Mover a...</option>
              {SALES_STAGES.map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
            </select>
          )}
          <button onClick={() => { setBatchMode(!batchMode); setSelectedIds(new Set()); }} className={`px-3 py-2 rounded-lg border text-xs font-medium ${batchMode ? "border-[#7C1D2E] bg-[#7C1D2E]/10 text-[#7C1D2E]" : "border-[#E8E0D8] text-[#9C8A82] hover:text-[#3D2B1F]"}`}>
      ...
      <div className="flex flex-col sm:flex-row gap-3 mb-6">
        <div className="relative flex-1">
          ...
          <input type="text" value={searchQuery} onChange={e => setSearchQuery(e.target.value)} placeholder="Buscar cliente..." className="w-full h-12 pl-12 pr-4 rounded-xl border border-[#E8E0D8] bg-white text-[#3D2B1F] placeholder-[#9C8A82] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30 focus:border-[#7C1D2E] transition-all" />
        </div>
          ...
          {(["", "comprador", "negocio"] as const).map(type => {
            ...
            return (
              <button key={type} onClick={() => setClientTypeFilter(type)}
                className={`h-10 sm:h-12 px-3 sm:px-4 rounded-xl border text-xs sm:text-sm font-medium flex items-center gap-1.5 sm:gap-2 transition-all ${clientTypeFilter === type ? "bg-[#7C1D2E] text-white border-[#7C1D2E]" : "border-[#E8E0D8] bg-white text-[#3D2B1F] hover:bg-[#FDF8F3]"}`}>
          ...
          {clientTypeFilter !== "negocio" && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                <CartIcon size={18} className="text-[#5B9E6B]" />
                <h2 className="text-sm font-semibold text-[#3D2B1F]">Clientes Compradores</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {SALES_STAGES.map(stage => {
                  ...
                  return (
                    <div
                      key={stage.key}
                      className={`bg-[#FDF8F3] rounded-xl p-3 min-h-[200px] transition-colors ${isDragOver ? "ring-2 ring-[#7C1D2E] bg-[#7C1D2E]/5" : ""}`}
                    ...
                    >
                      <div className="flex items-center justify-between mb-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-xs font-semibold text-[#3D2B1F]">{stage.label}</span>
                        </div>
          ...
          {clientTypeFilter !== "comprador" && (
            <div>
              <div className="flex items-center gap-2 mb-4">
                ...
                <h2 className="text-sm font-semibold text-[#3D2B1F]">Prospectos de Negocio</h2>
              ...
              <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {RECRUITMENT_STAGES.map(stage => {
                  ...
                  return (
                    <div
                      key={stage.key}
                      className={`bg-[#FDF8F3] rounded-xl p-3 min-h-[200px] transition-colors ${isDragOver ? "ring-2 ring-purple-500 bg-purple-50/50" : ""}`}
                      onDragOver={e => handleDragOver(e, stage.key)}
                      onDragLeave={handleDragLeave}
                      onDrop={e => handleDrop(e, stage.key)}
                    >
        ...
        {selectedClient && (
          <div className="space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {selectedClient.tags.map(t => (
                  <span key={t.id} className="text-xs px-2 py-0.5 rounded-full bg-[#7C1D2E]/10 text-[#7C1D2E] font-medium">{t.name}</span>
                ))}
              </div>
              <div className="flex items-center gap-2 flex-wrap">
                {(selectedClient.client_type || "comprador") === "comprador" ? (
                  ...
                  <button onClick={() => handleConvertClientType(selectedClient.id, "comprador")} className="flex items-center gap-1.5 text-xs text-[#5B9E6B] hover:underline px-2 py-1 rounded-lg hover:bg-[#5B9E6B]/10">
                ...
                <select value={selectedClient.stage} onChange={e => handleStageChange(selectedClient.id, e.target.value)}
                  className="h-9 px-3 rounded-lg border border-[#E8E0D8] bg-white text-xs text-[#3D2B1F] focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30">
            ...
            {selectedClient.previous_client_type && selectedClient.client_type_changed_at && (
              <div className="bg-[#FDF8F3] rounded-xl p-3 flex items-center gap-3">
                <div className="w-8 h-8 rounded-full bg-[#7C1D2E]/10 flex items-center justify-center">
                  <Briefcase size={14} className="text-[#7C1D2E]" />
                </div>
                <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {QUALIFICATION_OPTIONS.map(q => (
                    <button key={q.key} onClick={() => handleStageChange(selectedClient.id, "calificacion", { qualification_level: q.key })}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedClient.qualification_level === q.key ? `${q.bg} ${q.color} ring-2 ring-offset-1 ring-current` : "bg-white text-[#9C8A82] border border-[#E8E0D8] hover:bg-[#FDF8F3]"}`}>
                      {q.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {CLOSURE_RESULTS.map(cr => (
                    <button key={cr.key} onClick={() => handleStageChange(selectedClient.id, "cierre", { closure_result: cr.key })}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedClient.closure_result === cr.key ? `${cr.bg} ${cr.color} ring-2 ring-offset-1 ring-current` : "bg-white text-[#9C8A82] border border-[#E8E0D8] hover:bg-[#FDF8F3]"}`}>
                      {cr.label}
                    </button>
                  ))}
                </div>
                ...
                <div className="flex flex-wrap gap-2">
                  {ENROLLMENT_RESULTS.map(er => (
                    <button key={er.key} onClick={() => handleStageChange(selectedClient.id, "inscripcion", { closure_result: er.key })}
                      className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${selectedClient.closure_result === er.key ? `${er.bg} ${er.color} ring-2 ring-offset-1 ring-current` : "bg-white text-[#9C8A82] border border-[#E8E0D8] hover:bg-[#FDF8F3]"}`}>
                      {er.label}
                    </button>
                  ))}
                </div>
            ...
            {selectedClient.days_in_stage !== null && selectedClient.days_in_stage >= STAGNATION_THRESHOLD_DAYS && (
              <div className="bg-[#FFFBEB] border border-[#D4A017]/30 rounded-xl p-4 flex items-center gap-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <div className="space-y-2">
                ...
                {selectedClient.phone && (
                  <p className="text-sm text-[#3D2B1F] flex items-center gap-2">
                    <Phone size={14} className="text-[#9C8A82]" /> {selectedClient.phone}
                    <a href={`https://wa.me/1${selectedClient.phone.replace(/\D/g, "")}`} target="_blank" rel="noopener noreferrer" className="text-[#5B9E6B] hover:underline text-xs"><MessageSquare size={12} /> WhatsApp</a>
                  </p>
                )}
                {selectedClient.email && <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><Mail size={14} className="text-[#9C8A82]" /> {selectedClient.email}</p>}
                {selectedClient.lead_source && <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><UserPlus size={14} className="text-[#9C8A82]" /> Llegó por: {selectedClient.lead_source}</p>}
                {selectedClient.interest && <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><Tag size={14} className="text-[#9C8A82]" /> Interés: {selectedClient.interest}</p>}
                {selectedClient.ibo_number && <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><UserCheck size={14} className="text-[#9C8A82]" /> IBO: {selectedClient.ibo_number}</p>}
              </div>
              <div className="space-y-2">
                <h4 className="text-xs font-semibold text-[#9C8A82] uppercase tracking-wider">Comercial</h4>
                <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><DollarSign size={14} className="text-[#9C8A82]" /> Total gastado: <strong>{formatCurrency(selectedClient.total_spent)}</strong></p>
                <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><ShoppingCart size={14} className="text-[#9C8A82]" /> Compras: <strong>{selectedClient.num_purchases}</strong></p>
                <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><BarChart3 size={14} className="text-[#9C8A82]" /> PV total: <strong>{selectedClient.pv_total.toFixed(2)}</strong></p>
                <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><TrendingUp size={14} className="text-[#9C8A82]" /> Ticket promedio: <strong>{formatCurrency(selectedClient.avg_ticket)}</strong></p>
                {selectedClient.repurchase_date && (
                  <p className="text-sm text-[#3D2B1F] flex items-center gap-2"><Zap size={14} className="text-[#5B9E6B]" /> Recompra: <strong>{formatDate(selectedClient.repurchase_date)}</strong></p>
                )}
              </div>
            </div>

            {selectedClient.pending_balance > 0 && (
              <div className="bg-[#FFF5F5] border border-[#E07A3A]/30 rounded-xl p-4">
                <p className="text-sm font-semibold text-[#E07A3A] flex items-center gap-2"><DollarSign size={16} /> Saldo pendiente: {formatCurrency(selectedClient.pending_balance)}</p>
              </div>
            )}
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-2">
                  {selectedClient.top_products.map((p, i) => (
                    <span key={i} className="text-xs px-3 py-1.5 rounded-full bg-[#FDF8F3] text-[#3D2B1F] border border-[#E8E0D8]">{p.name} ({p.count}x)</span>
                  ))}
                </div>
              </div>
            )}
            ...
            {riskScore && (
              <div className={`rounded-xl p-4 border ${riskScore.level === "high" ? "bg-[#FFF5F5] border-[#E07A3A]/30" : riskScore.level === "medium" ? "bg-[#FFFBEB] border-[#D4A017]/30" : "bg-[#F0FAF4] border-[#5B9E6B]/30"}`}>
                <div className="flex items-center justify-between mb-2">
                  <h4 className="text-xs font-semibold uppercase tracking-wider">Análisis de Riesgo</h4>
                  <span className={`text-xs font-bold px-2 py-0.5 rounded-full ${riskScore.level === "high" ? "bg-[#E07A3A]/20 text-[#E07A3A]" : riskScore.level === "medium" ? "bg-[#D4A017]/20 text-[#B8860B]" : "bg-[#5B9E6B]/20 text-[#6DB08A]"}`}>
                    {riskScore.level === "high" ? "Alto" : riskScore.level === "medium" ? "Medio" : "Bajo"}
                  </span>
                </div>
                <div className="space-y-1">
                  {riskScore.factors.map((f, i) => (
                    <div key={i} className="flex items-center justify-between text-xs text-[#3D2B1F]">
                      <span>{f.label}</span>
                      <span className={`font-medium ${f.impact > 30 ? "text-[#E07A3A]" : f.impact > 10 ? "text-[#D4A017]" : "text-[#5B9E6B]"}`}>+{f.impact}%</span>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div>
              ...
              }} disabled={loadingSummary} className="w-full h-10 bg-[#7C1D2E]/10 text-[#7C1D2E] rounded-xl text-sm font-medium hover:bg-[#7C1D2E]/20 transition-all disabled:opacity-50">
              ...
              {aiSummary && (
                <div className="mt-3 space-y-3">
                  <div className="bg-[#FDF8F3] rounded-xl p-4">
                    ...
                    <p className="text-sm text-[#3D2B1F] leading-relaxed whitespace-pre-wrap">{aiSummary}</p>
                  </div>
                  {approach && (
                    <div className="bg-[#7C1D2E]/5 border border-[#7C1D2E]/20 rounded-xl p-4">
                      <h4 className="text-xs font-semibold text-[#7C1D2E] uppercase tracking-wider mb-2">Sugerencia de Abordaje</h4>
                      <p className="text-sm text-[#3D2B1F] leading-relaxed">{approach}</p>
                    </div>
                  )}
                </div>
              )}
            ...
            {selectedClient.notes && (
              <div>
                ...
                <p className="text-sm text-[#3D2B1F] bg-[#FDF8F3] rounded-xl p-3">{selectedClient.notes}</p>
              </div>
            )}
          </div>
        )}
      ...
      <Modal isOpen={showMigrateModal} onClose={() => setShowMigrateModal(false)} title="Migrar etapas del pipeline">
        <div className="space-y-4">
          <p className="text-sm text-[#3D2B1F]">Se detectaron clientes con etapas del sistema anterior. Se migrarán automáticamente:</p>
          ...
          <div className="flex gap-3">
            <button onClick={() => setShowMigrateModal(false)} className="flex-1 h-12 border border-[#E8E0D8] text-[#3D2B1F] rounded-xl text-sm font-medium hover:bg-[#FDF8F3]">Cancelar</button>
            <button onClick={handleMigrate} disabled={migrating} className="flex-1 h-12 bg-[#7C1D2E] text-white rounded-xl text-sm font-medium hover:bg-[#5C1420] disabled:opacity-50">{migrating ? "Migrando..." : "Migrar ahora"}</button>
          </div>
        </div>
      </Modal>
      ...
      <Modal isOpen={showAddModal} onClose={() => setShowAddModal(false)} title="Agregar al Pipeline">
        <div className="space-y-4">
          <div className="flex gap-3">
            {([["comprador", "Cliente Comprador", CartIcon], ["negocio", "Prospecto de Negocio", BriefcaseIcon]] as const).map(([value, label, Icon]) => (
              <button key={value} type="button" onClick={() => {
                ...
                className={`flex-1 flex items-center gap-2 p-3 rounded-xl border text-sm font-medium transition-all ${addForm.client_type === value ? "border-[#7C1D2E] bg-[#7C1D2E]/5 text-[#3D2B1F]" : "border-[#E8E0D8] text-[#9C8A82] hover:bg-[#FDF8F3]"}`}>
                <Icon size={16} /> {label}
              </button>
            ))}
          </div>
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Etapa inicial</label>
            <select value={addForm.stage} onChange={e => setAddForm({ ...addForm, stage: e.target.value })}
              className="w-full h-12 px-4 rounded-xl border border-[#E8E0D8] bg-white text-[#3D2B1F] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30">
          ...
          <div>
            ...
            <input type="text" value={addForm.full_name} onChange={e => setAddForm({ ...addForm, full_name: e.target.value })} placeholder="Nombre y apellidos" className="w-full h-12 px-4 rounded-xl border border-[#E8E0D8] bg-[#FCFAF7] text-[#3D2B1F] placeholder-[#9C8A82] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30" />
          </div>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Teléfono</label>
              <input type="text" value={addForm.phone} onChange={e => setAddForm({ ...addForm, phone: e.target.value })} placeholder="809-000-0000" className="w-full h-12 px-4 rounded-xl border border-[#E8E0D8] bg-[#FCFAF7] text-[#3D2B1F] placeholder-[#9C8A82] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30" />
            </div>
            <div>
              <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Email</label>
              <input type="email" value={addForm.email} onChange={e => setAddForm({ ...addForm, email: e.target.value })} placeholder="correo@ejemplo.com" className="w-full h-12 px-4 rounded-xl border border-[#E8E0D8] bg-[#FCFAF7] text-[#3D2B1F] placeholder-[#9C8A82] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30" />
            </div>
          </div>
          <div className="flex gap-3 pt-2">
            <button onClick={() => setShowAddModal(false)} className="flex-1 h-12 border border-[#E8E0D8] text-[#3D2B1F] rounded-xl text-sm font-medium hover:bg-[#FDF8F3]">Cancelar</button>
            <button onClick={handleAddToPipeline} disabled={saving} className="flex-1 h-12 bg-[#7C1D2E] text-white rounded-xl text-sm font-medium hover:bg-[#5C1420] disabled:opacity-50 flex items-center justify-center gap-2">
