    ...
    <PageContainer>
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        <div className="lg:col-span-2">
          <div className="flex items-center justify-between mb-4">
            <div>
              <h1 className="text-xl font-bold text-[#3D2B1F]">Clientes y Deudas</h1>
              <p className="text-sm text-[#9C8A82] mt-1">Directorio de clientes</p>
            </div>
            <button onClick={openNew} className="flex items-center gap-2 bg-[#7C1D2E] text-white px-5 py-2.5 rounded-xl text-sm font-medium hover:bg-[#5C1420] transition-all shadow-sm">
              <Plus size={18} /> Añadir
            </button>
          </div>
          ...
          <div className="relative mb-4">
            ...
            <input type="text" value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} placeholder="Buscar cliente por nombre, teléfono o correo..." className="w-full h-12 pl-12 pr-4 rounded-xl border border-[#E8E0D8] bg-white text-[#3D2B1F] placeholder-[#9C8A82] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30 focus:border-[#7C1D2E] transition-all" />
          </div>
                    ...
                    <div className="flex items-start justify-between">
                      <button onClick={() => openDetail(client)} className="flex-1 text-left">
                        <div className="flex items-center gap-2 mb-1">
                          <h3 className="font-medium text-[#3D2B1F] hover:text-[#7C1D2E] transition-colors">{client.full_name}</h3>
                          {stage && (
                        ...
                        <div className="flex items-center gap-4 mt-2">
                          {pending > 0 ? (
                            <>
                              <span className="text-sm text-[#3D2B1F]">Pendiente: <strong>{formatCurrency(pending)}</strong></span>
                              <Badge variant="danger">DEBE {formatCurrency(pending)}</Badge>
                            </>
                          ) : credit > 0 ? (
                            <>
                              <span className="text-sm text-[#3D2B1F]">A favor: <strong>{formatCurrency(credit)}</strong></span>
                              <Badge variant="success">A FAVOR {formatCurrency(credit)}</Badge>
                            </>
                          ) : (
                            <>
                              <span className="text-sm text-[#3D2B1F]">Saldo: <strong>{formatCurrency(0)}</strong></span>
                      ...
                      <div className="flex items-center gap-2 ml-4">
                        <select
                          ...
                          className="h-8 px-2 rounded-lg border border-[#E8E0D8] bg-white text-xs text-[#3D2B1F] focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30"
                        >
                          <option value="">Sin etapa</option>
                          {getStagesForType((client.client_type as ClientType) || "comprador").map(s => <option key={s.key} value={s.key}>{s.label}</option>)}
                        </select>
                        <button onClick={() => openEdit(client)} className="p-2 text-[#9C8A82] hover:bg-[#FDF8F3] rounded-lg"><Edit2 size={16} /></button>
                        <button onClick={() => handleDelete(client.id, client.full_name)} className="p-2 text-[#E07A3A] hover:bg-[#E07A3A]/10 rounded-lg"><Trash2 size={16} /></button>
                      </div>
                    </div>
                  </div>
                );
        ...
        <div className="bg-white rounded-2xl p-5 shadow-sm border border-[#E8E0D8] h-fit">
          <h3 className="text-sm font-semibold text-[#3D2B1F] mb-4">Estado de Cuenta Doña Nina</h3>
          <div className="space-y-3 mb-6">
            ...
            <div className="flex justify-between text-sm py-2 border-b border-[#E8E0D8]/50">
              ...
              <span className="font-medium text-[#5B9E6B]">{formatCurrency(totalCreditBalance)}</span>
            </div>
          </div>

          <h4 className="text-xs font-semibold text-[#9C8A82] uppercase tracking-wider mb-3">Accesos rápidos</h4>
          <div className="space-y-2">
            <a href="/creditos" className="flex items-center gap-2 text-sm text-[#5B9E6B] hover:underline">
              <Wallet size={14} /> Ver Saldos a Favor
            </a>
            <a href="/crm" className="flex items-center gap-2 text-sm text-[#7C1D2E] hover:underline">
          ...
          <div className="space-y-5">
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
              <div className="flex items-center gap-4">
                <div className="w-12 h-12 rounded-full bg-[#7C1D2E]/10 flex items-center justify-center">
                  <User size={22} className="text-[#7C1D2E]" />
                </div>
                <div>
              ...
              <div className="flex items-center gap-3 flex-wrap">
                {detailClient.client_type === "comprador" ? (
                  <button 
                    ...
                    className="flex items-center gap-1.5 text-sm text-[#5B9E6B] hover:underline"
                  >
                  ...
                  <button 
                    ...
                    className="flex items-center gap-1.5 text-sm text-[#7C1D2E] hover:underline"
                  >
                    <User size={14} /> <span className="hidden sm:inline">Convertir a Comprador</span><span className="sm:hidden">Comprador</span>
                  </button>
                )}
                <button onClick={() => openEdit(detailClient)} className="flex items-center gap-1.5 text-sm text-[#7C1D2E] hover:underline"><Edit2 size={14} /> Editar</button>
              </div>
            </div>
              ...
              {(["info", "facturas", "pagos", "creditos", "seguimiento"] as DetailTab[]).map((tab) => (
                <button key={tab} onClick={() => setDetailTab(tab)}
                  className={`pb-2.5 px-3 text-xs font-medium whitespace-nowrap transition-colors border-b-2 ${
                    detailTab === tab ? "text-[#7C1D2E] border-[#7C1D2E]" : "text-[#9C8A82] border-transparent hover:text-[#3D2B1F]"
                  }`}
                >
            ...
            {detailLoading ? (
              <div className="flex justify-center py-10">
                <div className="w-6 h-6 border-2 border-[#7C1D2E] border-t-transparent rounded-full animate-spin" />
              </div>
            ) : detailTab === "info" ? (
              <div className="space-y-3">
                {detailClient.notes && (
                  <div className="bg-[#FDF8F3] rounded-xl p-4">
                    ...
                    <p className="text-sm text-[#3D2B1F]">{detailClient.notes}</p>
                  </div>
                )}
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-white rounded-xl p-4 border border-[#E8E0D8]">
                    <p className="text-xs text-[#9C8A82]">Total facturado</p>
                    <p className="text-lg font-bold text-[#3D2B1F]">{formatCurrency(totalInvoiced)}</p>
                  </div>
                  <div className="bg-white rounded-xl p-4 border border-[#E8E0D8]">
                    <p className="text-xs text-[#9C8A82]">Total pagado</p>
                    <p className="text-lg font-bold text-[#5B9E6B]">{formatCurrency(totalPaid)}</p>
                  </div>
                </div>
                <div className="bg-white rounded-xl p-4 border border-[#E8E0D8]">
                  <p className="text-xs text-[#9C8A82]">Saldo pendiente</p>
                  <p className="text-lg font-bold text-[#7C1D2E]">{formatCurrency(Math.max(0, totalInvoiced - totalPaid))}</p>
                </div>
              </div>
            ) : detailTab === "facturas" ? (
              <div className="space-y-2">
                {detailInvoices.length === 0 ? (
                  ...
                  detailInvoices.map((inv: any) => (
                    <div key={inv.id} className="flex items-center justify-between bg-white rounded-xl p-3 border border-[#E8E0D8]">
                      <div>
                        <p className="text-sm font-medium text-[#3D2B1F]">{inv.invoice_number}</p>
                        <p className="text-xs text-[#9C8A82]">{formatDate(inv.invoice_date)}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-sm font-medium text-[#3D2B1F]">{formatCurrency(inv.total)}</p>
                ...
                {detailReceipts.length === 0 ? (
                  ...
                  detailReceipts.map((rec: any) => (
                    <div key={rec.id} className="flex items-center justify-between bg-white rounded-xl p-3 border border-[#E8E0D8]">
                      <div>
                        <p className="text-sm font-medium text-[#3D2B1F]">{rec.receipt_number}</p>
                        <p className="text-xs text-[#9C8A82]">
                      ...
                      <p className="text-sm font-medium text-[#5B9E6B]">{formatCurrency(rec.amount)}</p>
                    </div>
                  ))
                )}
              </div>
                ...
                {detailCredits.length === 0 ? (
                  ...
                  detailCredits.map((c: any) => (
                    <div key={c.id} className="flex items-center justify-between bg-white rounded-xl p-3 border border-[#E8E0D8]">
                      <div>
                        <p className="text-sm text-[#3D2B1F]">Recibo {c.receipts?.receipt_number || "—"}</p>
              ...
              <div className="space-y-3">
                <div className="flex gap-2">
                  <input
                    ...
                    className="flex-1 h-10 px-4 rounded-xl border border-[#E8E0D8] text-sm text-[#3D2B1F] placeholder-[#9C8A82] focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30"
                  />
                  <button onClick={handleAddFollowup} className="h-10 px-4 bg-[#7C1D2E] text-white rounded-xl text-sm font-medium hover:bg-[#5C1420] transition-all">
                    <Plus size={16} />
                  </button>
                </div>
                  ...
                  <div className="space-y-2">
                    {detailFollowups.map((f) => (
                      <div key={f.id} className="flex items-start gap-3 bg-white rounded-xl p-3 border border-[#E8E0D8]">
                        <div className="w-8 h-8 rounded-full bg-[#FDF8F3] flex items-center justify-center flex-shrink-0">
                          <MessageSquare size={14} className="text-[#7C1D2E]" />
                        </div>
                        <div className="flex-1">
                          <div className="flex items-center justify-between">
                            <p className="text-sm font-medium text-[#3D2B1F]">Seguimiento</p>
                            <button
                          ...
                          <p className="text-sm text-[#3D2B1F] mt-1">{f.comments}</p>
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); resetForm(); }} title={editingClient ? "Editar Cliente" : "Nuevo Cliente"} subtitle="Registra la información del cliente">
        <div className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Tipo de cliente *</label>
            <div className="flex gap-3">
              {([["comprador", "Cliente Comprador", "Compra productos"], ["negocio", "Prospecto de Negocio", "Posible IBO / Demo"]] as const).map(([value, label, desc]) => (
                <button key={value} type="button" onClick={() => setForm({ ...form, client_type: value as ClientType })}
                  className={`flex-1 p-3 rounded-xl border text-left transition-all ${form.client_type === value ? "border-[#7C1D2E] bg-[#7C1D2E]/5" : "border-[#E8E0D8] bg-white hover:bg-[#FDF8F3]"}`}>
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input type="text" value={form.phone} onChange={(e) => setForm({ ...form, phone: e.target.value })} placeholder="809-000-0000" className="w-full h-12 px-4 rounded-xl border border-[#E8E0D8] bg-[#FCFAF7] text-[#3D2B1F] placeholder-[#9C8A82] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30 focus:border-[#7C1D2E] transition-all" />
            </div>
            <div>
          ...
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Fecha de cumpleaños</label>
            <input type="date" value={form.birthday} onChange={(e) => setForm({ ...form, birthday: e.target.value })} className="w-full h-12 px-4 rounded-xl border border-[#E8E0D8] bg-[#FCFAF7] text-[#3D2B1F] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30 focus:border-[#7C1D2E] transition-all" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Número IBO (opcional)</label>
            <input type="text" value={form.ibo_number} onChange={(e) => setForm({ ...form, ibo_number: e.target.value })} placeholder="IBO" className="w-full h-12 px-4 rounded-xl border border-[#E8E0D8] bg-[#FCFAF7] text-[#3D2B1F] placeholder-[#9C8A82] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30 focus:border-[#7C1D2E] transition-all" />
          </div>
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Notas</label>
            <textarea value={form.notes} onChange={(e) => setForm({ ...form, notes: e.target.value })} placeholder="Información adicional del cliente..." rows={3} className="w-full px-4 py-3 rounded-xl border border-[#E8E0D8] bg-[#FCFAF7] text-[#3D2B1F] placeholder-[#9C8A82] text-sm focus:outline-none focus:ring-2 focus:ring-[#7C1D2E]/30 focus:border-[#7C1D2E] transition-all resize-none" />
          </div>
