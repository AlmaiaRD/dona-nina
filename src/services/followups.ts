...
export async function getFollowups(page = 1, pageSize = 500) {
  ...
  return data as (Followup & { client: { full_name: string } })[]
}
...
export async function getAllFollowups(page = 1, pageSize = 500) {
  const from = (page - 1) * pageSize
  const to = from + pageSize - 1
  const { data, error } = await supabase
    .from('followups')
