    ...
    <PageContainer>
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        ...
        {(filterMonth || filterYear || filterStatus !== "all") && (
          <button onClick={() => { setFilterMonth(""); setFilterYear(""); setFilterStatus("all"); }} className="text-xs text-[#9C8A82] hover:text-[#3D2B1F] px-3">Limpiar filtros</button>
        )}
      </div>
                  ...
                  <tr key={rec.id} className="bg-white rounded-xl shadow-sm border border-[#E8E0D8] hover:shadow-md transition-shadow">
                    ...
                    <td className="px-4 py-3.5">
                      <div className="flex items-center justify-center gap-1">
                        <button onClick={() => { setSelectedReceipt(rec); setShowDetail(true); }} className="p-2 text-[#9C8A82] hover:bg-[#FDF8F3] rounded-lg" title="Ver"><Eye size={15} /></button>
          ...
          <div className="space-y-5">
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-sm text-[#3D2B1F]">{formatDate(selectedReceipt.receipt_date || selectedReceipt.created_at)}</p>
              </div>
            </div>
            ...
            {selectedReceipt.concept && (
              <div>
                ...
                <p className="text-sm text-[#3D2B1F] bg-[#FDF8F3] rounded-xl p-3">{selectedReceipt.concept}</p>
              </div>
            )}
            ...
            <div className="flex flex-wrap gap-3">
              ...
              <button onClick={() => { setShowDetail(false); handlePrintJpg(selectedReceipt); }} className="flex-1 min-w-[120px] h-12 border border-[#E8E0D8] text-[#3D2B1F] rounded-xl text-sm font-medium hover:bg-[#FDF8F3] transition-all flex items-center justify-center gap-2">
      ...
      <Modal isOpen={showEditModal} onClose={() => { setShowEditModal(false); setSelectedReceipt(null); }} title="Editar Recibo" subtitle={selectedReceipt?.receipt_number || ""} wide>
        <div className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Monto</label>
              <input
          ...
          {editForm.payment_method === "TRANSFER" && (
            <div>
              <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Cuenta bancaria destino</label>
              <select
      ...
      <Modal isOpen={showModal} onClose={() => { setShowModal(false); resetForm(); }} title="Registrar Pago" subtitle={selectedInvoiceData?.clients?.full_name ? `Cliente: ${selectedInvoiceData.clients.full_name}` : undefined} wide>
        <div className="space-y-5">
          ...
          <div>
            <label className="block text-sm font-medium text-[#3D2B1F] mb-1.5">Fecha del recibo</label>
            <input
